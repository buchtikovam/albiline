<script lang="ts">
	...
	initialFormData.retailStoreTypeName = initialData.retailStoreTypeName || null;
	...
	initialFormData.sendDeliveryNoteViaEmail = initialData.sendDeliveryNoteViaEmail || null;
	initialFormData.emailForDeliveryNotes = initialData.emailForDeliveryNotes || null;
...
<div class="flex h-full gap-4 max-w-[1800px] p-4 overflow-auto">
	<div class="hidden lg:block sticky w-[200px] h-fit ">
		<p class="font-bold">Detail zákazníka <br/>č. {initialData.customerAddressCode}</p>
	...
	<form use:enhance class="w-full overflow-auto" method="POST">
		<div id="fakturacni-udaje" class="flex flex-col gap-2">
			...
			<FormContainer>
				<div class="hidden 2xl:flex flex-col gap-1">
					<div class="flex">
			...
			<FormContainer>
				...
				<div class="flex flex-col 2xl:hidden gap-1">
					...
					<div class="block md:flex gap-2">
						...
						<div class="flex w-full pl-0 md:pl-1">
							<FSCheckboxWrapper bind:value={$formData.consignmentSaldeEnabled} form={form} name="consignmentSaleEnabled" label="Kontrola u vratek" />
							<FSCheckboxWrapper bind:value={$formData.consignmentSadleEnabled} form={form} name="consignmentSaleEnabled" label="Požívat as. EANy" />
						</div>
					</div>  
		...
		<div id="popis" class="flex flex-col gap-2 mt-4">
			...
			<FormContainer>
				...
				<div class="hidden 2xl:flex flex-col gap-1">
					<FormSectionFull>
						<FSInputWrapper bind:value={$formData.invoiceCopies} form={form} name="invoiceCopies" label="Poznámka" />
						<FSInputWrapper bind:value={$formData.dueDays} form={form} name="dueDays" label="Bližší popis"  />
					</FormSectionFull>

					<FormSectionFull>
						<FSInputWrapper bind:value={$formData.dueDays} form={form} name="dueDays" label="Zodpovědná osoba"  />
						<FSInputWrapper bind:value={$formData.invoiceCopies} form={form} name="invoiceCopies" label="Poznámka pro balení" />
					</FormSectionFull>
				</div>
		...
		<div id="struktura" class="flex flex-col gap-2 mt-4">
			...
			<FormContainer>
				<div class="hidden 2xl:flex flex-col gap-1">
					<FormSectionFull>
						<FSInputWrapper bind:value={$formData.city} form={form} name="city" label="CustomerAddressId" />
						<FSInputWrapper bind:value={$formData.postalCode} form={form} name="postalCode" label="CustomerNodeCode" />
						<FSInputWrapper bind:value={$formData.countryCode} form={form} name="countryCode" label="AdressType" />
					</FormSectionFull>
				</div>
		...
		<div id="aviza" class="flex flex-col gap-2 mt-4">
			...
			<FormContainer>
				<FormSectionFull>
					<FSInputWrapper bind:value={$formData.city} form={form} name="city" label="Email pro avíza" />
					
					<div class="w-full flex">
						<FSCheckboxWrapper bind:value={$formData.consignmentSaldeEnabled} form={form} name="consignmentSaleEnabled" label="Avízo o svozu zásilky" />
						<FSCheckboxWrapper bind:value={$formData.consignmentSadleEnabled} form={form} name="consignmentSaleEnabled" label="Avízo o expedici obj." />
					</div>
